...
Porque existe: a auditoria de 2026-08-25 escreveu "robots OK (IA permitida)" para as duas
superficies da casa sem nomear um unico crawler. Assercao sem medicao. Este script mede, e
separa TREINO de CITABILIDADE, que nao sao a mesma coisa: bloquear GPTBot nao tira
citabilidade no ChatGPT Search (isso e o OAI-SearchBot), e Google-Extended nunca governa
elegibilidade para AI Overviews (isso e o Googlebot).

Taxonomia extraida de AgriciDaniel/claude-seo (MIT) a 2026-09-14, com as fontes primarias
abaixo. Sem dependencias externas: stdlib apenas, de proposito, para nao arrastar playwright.
...
UA = "Mozilla/5.0 (compatible; EcoGEO/1.0; +https://inubia.pt)"

# Lista canonica de bots de IA, copia LOCAL de ai-robots-txt/ai.robots.txt (MIT, 68 contribuidores).
# Local de proposito: o auditor nao vai a rede em cada execucao, senao a medicao deixa de ser
# reproduzivel e passa a depender de um repo alheio estar de pe. Actualizar com actualiza_bots.sh.
# Serve para 2 coisas: dizer quais NAO obedecem ao robots.txt (permitir/bloquear nao os trava) e
# assinalar bots novos que a nossa tabela de 13 ainda nao conhece.
_CANON = {}
try:
    import os as _os
...
BOTS = [
    ...
    # Acrescentados 2026-09-14 ao cruzar com a lista canonica: sao de CITABILIDADE e faltavam.
...
def permitido(regras, caminho="/"):
    ...
    for tipo, valor in regras:
        if valor == "":
            # "Disallow:" vazio significa permitir tudo; "Allow:" vazio nao restringe.
            if tipo == "disallow":
                continue
            continue
...
def audita(alvo):
    ...
    # ausencia de medicao, e este script existe precisamente porque a casa ja leu uma
    # ausencia como facto uma vez (auditoria de 2026-08-25).
    if st == 200:
    ...
    # Bots novos: so os de operadores que decidem se somos citados. A lista canonica tem 175
    # entradas e a maioria e scraper obscuro: despejar tudo seria ruido. O sinal util e outro,
    # e e raro: um operador que importa lancou um bot que a nossa tabela ainda nao conhece.
...
def imprime(r):
    ...
    if r.get("bots_novos"):
        print(f"\n  BOTS DE OPERADORES QUE IMPORTAM e que a nossa tabela ainda NAO cobre ({len(r['bots_novos'])}):")
    ...
    print("    peso ZERO para Google Search (guia oficial 2026-06-29). Reportar, nunca prometer.")
    h = r["home"]
    if "erro" not in h:
